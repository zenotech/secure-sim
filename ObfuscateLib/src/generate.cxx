...
int main(int argc, char *argv[]){
	...
	if(argc != 2){
		cout << "Usage: generate key_characters"  << endl;
		exit(EXIT_FAILURE);
	}
	...
	if(key.size() < 1){
		cout << "Error: key less than 1 character" << endl;
		exit(EXIT_FAILURE);
	}
	...
	unsigned char obfuscatedSecretKey[key.size()];

	for(int i = 0; i < key.size(); ++i){
		if (i > SHA384_DIGEST_LENGTH) {
			obfuscatedSecretKey[i] = key[i] ^ hash[i - SHA384_DIGEST_LENGTH];
		}
		else { 
	...
	unsigned char originalKey[key.size()]; 
	for(int i = 0; i < key.size(); ++i){
		if (i > SHA384_DIGEST_LENGTH) {
			originalKey[i] = obfuscatedSecretKey[i] ^ hash[i - SHA384_DIGEST_LENGTH];
		}
		else { 
